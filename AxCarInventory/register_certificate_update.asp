...
result = Axapta.CallStaticClassMethodEx("CISKiaDealerWeb","UpdateCarCertificateRequestV2",axParmList)
...
if result <> "" then
  ...
  'Set objNewMail_NH     = CreateObject("CDO.Message")
    ...
    if request.Form("return_Id") = 3 then
        Response.Redirect("/AxCarInventory/aktivlistenV3.aspx?m=3")
    end if
    if request.Form("return_Id") = 2 then
        Response.Redirect("/AxCarInventory/aktivlistenV3.aspx?m=1")
    end if
    if request.Form("return_Id") = 1 then
        Response.Redirect("/AxSearch/carSearch.asp")
    end if
    Response.End() 
end if
%>